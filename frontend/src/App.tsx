...
export default function App() {
  ...
  const liveName = `${formatReleaseName(metadata, settings)}.mkv`
  const firstFile = renamer.plan?.items[0]
...
function RenameRow({ item }: { item: RenameItem }) {
  ...
  return <div className="rename-row"><div className="path-cell old"><span className="path-icon"><Icon name={item.kind === 'folder' ? 'folder' : 'file'} size={15} /></span><span title={item.oldPath}>{oldName}</span></div><Icon name="arrow" size={15} /><div className="path-cell new"><span className="path-icon"><Icon name="file" size={15} /></span><span title={item.newPath}>{newName}</span></div><StatusBadge status={item.status} /></div>
}
...
function HistoryView({ plan }: { plan: RenamePlan | null }) {
  return <section className="panel full-workspace-panel"><div className="panel-heading"><div><span className="panel-kicker">HISTORY</span><h2>Latest rename plan</h2></div><span className="source-chip">Local journal</span></div>{plan ? <div className="rename-list">{plan.items.map((item) => <RenameRow key={`${item.oldPath}-${item.newPath}`} item={{ ...item, status: item.status === 'ready' ? 'same' : item.status }} />)}</div> : <div className="preview-empty"><Icon name="history" size={18} /><span>Applied transactions will appear here after the first rename.</span></div>}</section>
}
